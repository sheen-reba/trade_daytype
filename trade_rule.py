# -*- coding: utf-8 -*-
dire='D:/projects/trade_daytype/trade_rule/'
mode=(3, 0)
gtype=0
...
def __dump(name,data,dump,pro):
    f=open(name,'wb')
    dump(data,f,protocol=pro)
    f.close()

def main(mod:tuple[int,int],gtyp:int=0,ori='trade_rule/',
         pre0={
    ...
    from pickle import load,loads,dump,HIGHEST_PROTOCOL
    import datetime #datetime给数据中的时间数据提供实例化支持
    if gtyp>=1:
        import lzma
    workdir=getcwd().replace('\\','/')+'/' #'D:\\projects\\trade_daytype'
    self=abspath(__file__).replace('\\','/') #'D:\\projects\\trade_daytype\\trade_rule.py'
    selfdir=self[:self.rfind('/')+1]
    objdir=workdir+ori
    if gtyp==2:
        from requests import get
        from base64 import b64decode
        retu=get(f'https://api.github.com/repos/sheen-reba/trade_daytype/contents/{pre0[mode]}?ref=main')
        if retu.status_code==200:
            data=b64decode(retu.json()['content'])
            makedirs(objdir,exist_ok=True) #万一指定路径不存在就会报错
            f=open(objdir+pre0[mode],'wb')
            f.write(data)
            f.close()
            __change_self(objdir,mode,gtype,self)
            print(f'{datetime.datetime.now()} 模块-trade_rule-从指定连接更新给定数据成功')
        return
    if exists(objdir): #先检查工作路径下是否有该数据文件夹
        if exists(objdir+pre0[mod]): #数据文件夹中是否有数据文件
            __change_self(objdir,mod,gtyp,self)
            return __load(objdir+pre0[mod],load)
        elif gtyp==1 and exists(objdir+pre1[mod]):
            data=__load(objdir+pre1[mod],loads,lzma.LZMADecompressor(format=lzma.FORMAT_XZ).decompress)
            __dump(objdir+pre0[mod],data,dump,HIGHEST_PROTOCOL)
            __change_self(objdir,mod,gtyp,self)
            return data
    if exists(workdir+pre0[mod]): #工作路径下是否有数据文件呢
        ...
        data=__load(workdir+pre1[mod],loads,lzma.LZMADecompressor(format=lzma.FORMAT_XZ).decompress)
        makedirs(objdir,exist_ok=True)
        __dump(objdir+pre0[mod],data,dump,HIGHEST_PROTOCOL)
        __change_self(objdir,mod,gtyp,self)
        return data
    ...
    if dire!=objdir and exists(dire): #默认路径下是否有数据文件夹呢（如果默认路径与工作路径不一致的话）
        if exists(dire+pre0[mod]):
            ...
            data=__load(dire+pre1[mod],loads,lzma.LZMADecompressor(format=lzma.FORMAT_XZ).decompress)
            makedirs(objdir,exist_ok=True)
            __dump(objdir+pre0[mod],data,dump,HIGHEST_PROTOCOL)
            __change_self(objdir,mod,gtyp,self)
            return data
    if selfdir!=objdir and selfdir!=dire and exists(selfdir):
        if exists(selfdir+pre0[mod]):
            ...
            data=__load(selfdir+pre1[mod],loads,lzma.LZMADecompressor(format=lzma.FORMAT_XZ).decompress)
            makedirs(objdir,exist_ok=True)
            __dump(objdir+pre0[mod],data,dump,HIGHEST_PROTOCOL)
            __change_self(objdir,mod,gtyp,self)
            return data
    ...
    if not gtyp:
        from requests import get
        from base64 import b64decode
        retu=get(f'https://api.github.com/repos/sheen-reba/trade_daytype/contents/{pre0[mod]}?ref=main')
        if retu.status_code==200:
            data=b64decode(retu.json()['content'])
        ...
        from requests import get
        from base64 import b64decode
        retu=get(f'https://api.github.com/repos/sheen-reba/trade_daytype/contents/{pre1[mod]}?ref=main')
        if retu.status_code==200:
            xz=b64decode(retu.json()['content']) #既然是压缩模式就把压缩文件也保存下
            f=open(objdir+pre1[mod],'wb')
            f.write(xz)
            f.close()
            data=loads(lzma.LZMADecompressor(format=lzma.FORMAT_XZ).decompress(xz))
            makedirs(objdir,exist_ok=True)
            __dump(objdir+pre0[mod],data,dump,HIGHEST_PROTOCOL)
            __change_self(objdir,mod,gtyp,self)
            return data
